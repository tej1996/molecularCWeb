...
try:
	...
	if num_rows_userid == 1:
		...
		if num_rows_drive == 0:
			...
			lv_s,lv_o = commands.getstatusoutput('sudo lvcreate  -V'+d_size+'G  --name  ' + d_name + '  --thin  m_vol_grp/m_pool1  -y')
			if lv_s != 0:
				...
				if in_s!=0:
					...
					dtype="expandable"
					dbconnect.cursor.execute("INSERT into users_staas(uid,drivename,drivesize,dtype) VALUES(%s,%s,%s,%s)",(rows_userid[0][0], d_name, d_size+' GB',dtype))
